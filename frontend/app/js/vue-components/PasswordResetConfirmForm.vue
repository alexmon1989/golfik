<template>
  <div>
    <form @submit.prevent="validate" id="password-reset-confirm-form" class="g-py-15" action="" method="post">
      ...
      <div
        :class="{'u-has-error-v1': errors.has('new_password1') || (newpassword1Errors.length > 0 && showServerErrors) }"
        class="mb-4">
        ...
        <input v-model="newpassword1"
               ...
               type="password"
               placeholder="Пароль">
        <small v-if="errors.has('new_password1')" class="form-control-feedback">{{ errors.first('new_password1') }}
        </small>
        <small v-if="showServerErrors && newpassword1Errors.length > 0"
               class="form-control-feedback">{{ newpassword1Errors }}
        </small>
      </div>
      <div
        :class="{'u-has-error-v1': errors.has('new_password2') || (newpassword2Errors.length > 0 && showServerErrors) }"
        class="mb-4">
        ...
        <small v-if="errors.has('new_password2')" class="form-control-feedback">{{ errors.first('new_password2') }}
        </small>
        <small v-if="showServerErrors && newpassword2Errors.length > 0" class="form-control-feedback">{{
          newpassword2Errors }}
        </small>
      </div>
